    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          ...
          soapOutput.textContent = formatJson(data.soap_note);
          icdOutput.textContent = formatJson(data.icd_codes);
...
async function saveEncounter() {
  ...
  try {
    await apiRequest('POST', `/patients/${selectedPatientId}/encounters`, {
      transcript: latestResult.transcript,
      soap: latestResult.soap_note,
      icd_codes: latestResult.icd_codes || []
    });
...
async function exportPdf() {
  ...
  try {
    ...
    const pdfBlob = await apiRequest('POST', '/export?format=pdf', { soap_note: latestResult.soap_note }, true);
